...
<body><main id="main"><div class="top"><div class="top-brand"><span class="mark">KITCODE</span><span>MINI</span></div><button class="dashboard-button" id="dashboardButton" data-testid="companion-dashboard" type="button" data-ready="false">Dashboard</button></div><div class="body"><strong class="percent" id="percent">--</strong><span class="state" id="state">CONNECTING</span></div><div class="metrics" aria-label="Tracker metrics"><span class="metric"><span class="metric-label">=</span><strong class="metric-value" id="equalsValue">--</strong></span><span class="metric"><span class="metric-label">WORK</span><strong class="metric-value" id="workValue">--</strong></span><span class="metric"><span class="metric-label">IDLE</span><strong class="metric-value" id="idleValue">--</strong></span></div></main>${renderCompanionSwitcher('mini')}<script>
const API_BASE=${JSON.stringify(apiBase)};const percent=document.getElementById('percent'),state=document.getElementById('state'),equalsValue=document.getElementById('equalsValue'),workValue=document.getElementById('workValue'),idleValue=document.getElementById('idleValue'),main=document.getElementById('main'),dashboardButton=document.getElementById('dashboardButton');let lastActiveSeconds=null,lastIdleSeconds=null;
...
function render(summary){const reward=summary?.reward||{},global=summary?.global||{},tiers=reward.tiers||[],ready=tiers.some((tier)=>tier.status==='ready'),active=Math.max(0,Number(global.totalActiveSeconds)||0),idle=Math.max(0,Number(global.totalIdleSeconds)||0),tracking=Math.max(0,Number(global.trackingProjects)||0),working=lastActiveSeconds!==null&&active>lastActiveSeconds,idling=lastIdleSeconds!==null&&idle>lastIdleSeconds,value=Math.max(0,Math.min(100,(Number(reward.progress)||0)*100));percent.textContent=Math.round(value)+'%';state.textContent=ready?'BREAK READY':working?'WORKING':idling?'IDLE':tracking?'TRACKING':'NO PROJECTS';equalsValue.textContent=Math.max(0,Math.floor(Number(global.totalEquals??reward.totalEquals)||0));workValue.textContent=formatDuration(active);idleValue.textContent=formatDuration(idle);lastActiveSeconds=active;lastIdleSeconds=idle;dashboardButton.dataset.ready=ready?'true':'false';dashboardButton.textContent=ready?'Claim':'Dashboard';main.classList.remove('offline')}
function offline(){percent.textContent='--';state.textContent='OFFLINE';equalsValue.textContent='--';workValue.textContent='--';idleValue.textContent='--';dashboardButton.dataset.ready='false';dashboardButton.textContent='Dashboard';main.classList.add('offline')}
